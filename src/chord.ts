...
export function parseChord(input: string): Chord {
    ...
    if (raw.length > 1 && raw.includes('+')) {
        throw new Error(`Chord "${input}": "${raw}" is not a recognized key or modifier combo`);
    }
    if (shift && raw.length === 1) {
        throw new Error(
    ...
    if (key.length > 1 && !NAMED_KEYS.has(key)) {
        throw new Error(`Chord "${input}": "${key}" is not a known named key`);
    }
    return { key, shift };
}
