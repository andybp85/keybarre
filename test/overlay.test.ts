...
beforeEach(() => {
    ...
    document.getElementById('kb-overlay-style')?.remove()
})
...
describe('createHelpOverlay', () => {
    ...
    it('marks extras and overrides', () => {
        const overlay = createHelpOverlay(resolved)
        expect(overlay.element.querySelectorAll('tr.kb-extra')).toHaveLength(1)
        expect(overlay.element.querySelectorAll('tr.kb-override')).toHaveLength(1)
    })
    ...
    it('injects its style element once across instances', () => {
        ...
        createHelpOverlay(resolved)
        expect(document.querySelectorAll('#kb-overlay-style')).toHaveLength(1)
    })
})
