...
describe('resolveBindings', () => {
    ...
    it('drops an action named in unbind', () => {
        const resolved = resolveBindings(STANDARD_KEYMAP, { unbind: ['set-loop'] })
    ...
    it('throws on an override for an unknown action', () => {
        expect(() => resolveBindings(STANDARD_KEYMAP, { overrides: { 'no-such-action': parseChord('x') } })).toThrow(
            /unknown action "no-such-action"/,
        )
    })

    it('throws on unbinding an unknown action', () => {
        expect(() => resolveBindings(STANDARD_KEYMAP, { unbind: ['no-such-action'] })).toThrow(/unknown action "no-such-action"/)
    })

    it('throws on a chord conflict, naming both actions', () => {
        expect(() => resolveBindings(STANDARD_KEYMAP, { extras: [extra('my-thing', 'm')] })).toThrow(
            /toggle-click.*my-thing|my-thing.*toggle-click/,
        )
    })
